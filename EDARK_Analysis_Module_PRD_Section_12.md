...
- Full diagnostic suite per model type
...
- Implementation via `marginaleffects` package which is already listed as an optional dependency.
...
- Most likely candidates for async if needed: mixed model fitting with LRT p-values, large univariable screens, zip assembly for large exports.
